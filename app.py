...
async def run_task(task: str):
    try:
        print(f"[DEBUG] Starting task execution with input: {task}")
        
        # Get LLM completion
        print("[DEBUG] Getting LLM completion")
        response = get_completions(task)
        print(f"[DEBUG] LLM response received: {response}")
        
        # Parse task code and arguments
        print("[DEBUG] Parsing task code and arguments")
        task_code = response['name']
        arguments = response['arguments']
        print(f"[DEBUG] Task code: {task_code}")
        print(f"[DEBUG] Arguments: {arguments}")

        # Execute specific task function
        print(f"[DEBUG] Attempting to execute task {task_code}")
        
        if "A1" == task_code:
            print("[DEBUG] Before executing A1")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A1 arguments parsed: {parsed_args}")
            A1(**parsed_args)
            print("[DEBUG] After executing A1")
            
        if "A2" == task_code:
            print("[DEBUG] Before executing A2")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A2 arguments parsed: {parsed_args}")
            A2(**parsed_args)
            print("[DEBUG] After executing A2")
            
        if "A3" == task_code:
            print("[DEBUG] Before executing A3")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A3 arguments parsed: {parsed_args}")
            A3(**parsed_args)
            print("[DEBUG] After executing A3")
            
        if "A4" == task_code:
            print("[DEBUG] Before executing A4")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A4 arguments parsed: {parsed_args}")
            A4(**parsed_args)
            print("[DEBUG] After executing A4")
            
        if "A5" == task_code:
            print("[DEBUG] Before executing A5")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A5 arguments parsed: {parsed_args}")
            A5(**parsed_args)
            print("[DEBUG] After executing A5")
            
        if "A6" == task_code:
            print("[DEBUG] Before executing A6")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A6 arguments parsed: {parsed_args}")
            A6(**parsed_args)
            print("[DEBUG] After executing A6")
            
        if "A7" == task_code:
            print("[DEBUG] Before executing A7")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A7 arguments parsed: {parsed_args}")
            A7(**parsed_args)
            print("[DEBUG] After executing A7")
            
        if "A8" == task_code:
            print("[DEBUG] Before executing A8")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A8 arguments parsed: {parsed_args}")
            A8(**parsed_args)
            print("[DEBUG] After executing A8")
            
        if "A9" == task_code:
            print("[DEBUG] Before executing A9")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A9 arguments parsed: {parsed_args}")
            A9(**parsed_args)
            print("[DEBUG] After executing A9")
            
        if "A10" == task_code:
            print("[DEBUG] Before executing A10")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] A10 arguments parsed: {parsed_args}")
            A10(**parsed_args)
            print("[DEBUG] After executing A10")

        if "B12" == task_code:
            print("[DEBUG] Before executing B12")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] B12 arguments parsed: {parsed_args}")
            B12(**parsed_args)
            print("[DEBUG] After executing B12")
            
        if "B3" == task_code:
            print("[DEBUG] Before executing B3")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] B3 arguments parsed: {parsed_args}")
            B3(**parsed_args)
            print("[DEBUG] After executing B3")
            
        if "B5" == task_code:
            print("[DEBUG] Before executing B5")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] B5 arguments parsed: {parsed_args}")
            B5(**parsed_args)
            print("[DEBUG] After executing B5")
            
        if "B6" == task_code:
            print("[DEBUG] Before executing B6")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] B6 arguments parsed: {parsed_args}")
            B6(**parsed_args)
            print("[DEBUG] After executing B6")
            
        if "B7" == task_code:
            print("[DEBUG] Before executing B7")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] B7 arguments parsed: {parsed_args}")
            B7(**parsed_args)
            print("[DEBUG] After executing B7")
            
        if "B9" == task_code:
            print("[DEBUG] Before executing B9")
            parsed_args = json.loads(arguments)
            print(f"[DEBUG] B9 arguments parsed: {parsed_args}")
            B9(**parsed_args)
            print("[DEBUG] After executing B9")
