...
def A1(email="22F30027775@ds.study.iitm.ac.in"):
    print(f"[DEBUG A1] Starting A1 function with email: {email}")
    try:
        print("[DEBUG A1] Preparing subprocess command")
        command = ["uv", "run", "https://raw.githubusercontent.com/sanand0/tools-in-data-science-public/tds-2025-01/project-1/datagen.py", email]
        print(f"[DEBUG A1] Command to execute: {command}")
        
        print("[DEBUG A1] Starting subprocess")
        ...
        print("[DEBUG A1] Subprocess started, waiting for communication")
        
        stdout, stderr = process.communicate()
        print(f"[DEBUG A1] Process completed with return code: {process.returncode}")
        print(f"[DEBUG A1] stdout: {stdout}")
        print(f"[DEBUG A1] stderr: {stderr}")
        
        if process.returncode != 0:
            print(f"[DEBUG A1] Process failed with non-zero return code: {process.returncode}")
            raise HTTPException(status_code=500, detail=f"Error: {stderr}")
            
        print("[DEBUG A1] Process completed successfully")
        return stdout
        
    except subprocess.CalledProcessError as e:
        print(f"[DEBUG A1] CalledProcessError occurred: {str(e)}")
        print(f"[DEBUG A1] Error output: {e.stderr}")
        raise HTTPException(status_code=500, detail=f"Error: {e.stderr}")
    except Exception as e:
        print(f"[DEBUG A1] Unexpected error occurred: {str(e)}")
        print(f"[DEBUG A1] Error type: {type(e)}")
        raise HTTPException(status_code=500, detail=f"Unexpected error: {str(e)}")
    

def A2(prettier_version="prettier@3.4.2", filename="/data/format.md"):
    print(f"[DEBUG A2] Starting A2 function with parameters:")
    print(f"[DEBUG A2] prettier_version: {prettier_version}")
    print(f"[DEBUG A2] filename: {filename}")
    
    try:
        command = [r"C:\Program Files\nodejs\npx.cmd", prettier_version, "--write", filename]
        print(f"[DEBUG A2] Command prepared: {command}")
        
        print("[DEBUG A2] Starting subprocess execution")
        ...
        print("[DEBUG A2] Subprocess completed successfully")
        print(f"[DEBUG A2] Return code: {result.returncode}")
        print(f"[DEBUG A2] stdout: {result.stdout}")
        print(f"[DEBUG A2] stderr: {result.stderr}")
        print("Prettier executed successfully.")
        
    except subprocess.CalledProcessError as e:
        print(f"[DEBUG A2] CalledProcessError occurred")
        print(f"[DEBUG A2] Return code: {e.returncode}")
        print(f"[DEBUG A2] stdout: {e.stdout}")
        print(f"[DEBUG A2] stderr: {e.stderr}")
        print(f"[DEBUG A2] Command that failed: {e.cmd}")
        print(f"An error occurred: {e}")
        raise HTTPException(status_code=500, detail=f"Prettier error: {e.stderr}")
        
    except Exception as e:
        print(f"[DEBUG A2] Unexpected error occurred: {str(e)}")
        print(f"[DEBUG A2] Error type: {type(e)}")
        print(f"[DEBUG A2] Error traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Unexpected error: {str(e)}")
